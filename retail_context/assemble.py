...
def build(case_facts: CaseFacts, compressed: Compressed) -> AssembledContext:
    """Assemble the position-aware context: top -> middle -> bottom."""
    # 1. Top boundary — the case-facts block, exactly one trailing newline.
    case_block = case_facts.to_markdown().rstrip("\n") + "\n"

    # 2. Middle — resolved sections in declaration order (refund first; the
    #    audit regex checks the exact sequence).
    resolved_blocks: dict[str, str] = {}
    for issue_id in ("refund", "subscription"):
        if issue_id not in compressed.summaries:
            raise KeyError(
                f"compressed.summaries is missing required resolved issue {issue_id!r}"
            )
        summary_text = compressed.summaries[issue_id].text.strip()
        resolved_blocks[issue_id] = f"{RESOLVED_TITLES[issue_id]}\n\n{summary_text}\n"

    # 3. Bottom boundary — the active block, byte-exact body.
    ...
    # 4. Concatenate with blank lines between sections.
    markdown = "\n".join(
        [case_block, resolved_blocks["refund"], resolved_blocks["subscription"], active_block]
    )
